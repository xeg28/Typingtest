...
//                    System.out.println(Math.max(0, contentHeight - viewportHeight) / 30);
    ...
    private static void resultSceneChange(double wpm) {
        ...
        UserHelper.lastWPM = wpm;
        TestResults.wpmLabel.setText("You typed " + wpm + " wpm");
        Quote currentQuote = QuoteHelper.currentQuote;
        if(ServerController.isConnected() && UserHelper.currentUser != null && currentQuote != null) {
            RequestController.sendUserTestRequest(UserHelper.currentUser.getId(), currentQuote.getId());
            TypingTestHelper.setTextInResultTextArea();
            // possibly start timeline
        }
        if(ServerController.isConnected() && currentQuote != null) {
            TypingTestHelper.setTextInLeaderboardsTextArea();
            RequestController.sendLeaderboardRequest(currentQuote.getId());
        }
